# https://www.kaggle.com/tisho1313/pos-tagged-features
...
# define four sequences; grammatical description of a given word; passed as arguments to nltk.RegexpParser();
# collected the four nltk.RegexpParser objects. This list is used in function get_sequence_tags.
...
## defining functions to get features ##
...
#returns dictionary with features which are used later for classification
def get_sentence_features(sentens_in):
    #input sentence is transformed with stemmer
    stemmed_words = list()
    for w in sentens_in.split():
        stemmed_words.append(stemmer.stem(w)) #outputs the stem

    sentence = ' '.join(stemmed_words)
    word_tokens = nltk.wordpunct_tokenize(sentence) #tokenized

    taged_tokens = tagger.tag(word_tokens) #token string comes to the tagger; this tagged tokens is used to get features
...
#transform "y" lables as binary with values : [0,1,2]. I realy need this because our classifier will understand only these values.
...
#main method for extracting features
...
# clf = clf.best_estimator_.named_steps['bernoullinb']
# coef = clf.best_estimator_.named_steps['bernoullinb'].coef_
# best_alpha = clf.best_estimator_.named_steps['bernoullinb'].alpha
# print("Best cross-validation alpha: {:.2f}".format(best_alpha))
#
# from sklearn.feature_extraction import DictVectorizer as dict_vect
# import numpy as np
# # import mglearn
# #
...
# predicted_lables = clf.predict(X_valid)
#
...
# plt.figure()
# plot_confusion_matrix(cnf_matrix, X_valid, Y_valid)
#
# plt.show()